...
ANON="${SUPABASE_ANON_KEY:-$(grep -s '^SUPABASE_ANON_KEY=' /opt/nestandart/.env | cut -d= -f2)}"
if [ -z "$ANON" ]; then
  echo "⚠️  SUPABASE_ANON_KEY не задан. Экспортируй переменную или проверь /opt/nestandart/.env"
  exit 1
fi
...
if ssh -o BatchMode=yes -o ConnectTimeout=10 "$VPS" 'true' 2>/dev/null; then
  ...
  remote=$(ssh -o BatchMode=yes "$VPS" '
    ...
    # Telegram webhook
    T=$(grep "^TELEGRAM_BOT_TOKEN=" /opt/nestandart/.env | cut -d= -f2)
    ...
    GK=$(grep "^GEMINI_API_KEY=" /opt/nestandart/.env | cut -d= -f2-)
    ...
    [ "$gc" = "200" ] && echo "  ✅ мозг КотЭ: Gemini отвечает (бот думает)" || echo "  ❌ мозг КотЭ молчит: Gemini вернул $gc — нужен валидный ключ в /opt/nestandart/.env"
  ')
  echo "$remote"
